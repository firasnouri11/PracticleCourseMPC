classdef OptimalControler < handle
    ...
    methods (Access = public)
        ...
        function [x_seq, u_seq] = solve(obj, x_ref)
            quadprog_solved = 0;
            ...
            scaled_x_ref = -1 * [repmat(x_ref,101,1);zeros(100,1)];
            f = scaled_x_ref;
            options = optimoptions('quadprog', 'Display', 'none');
            [var_optim, ~, exitflag] = quadprog(obj.H, f, C_ineq1, C_ineq2, C_eq1, C_eq2, [], [], [], options);

            %zk = [1;-2;0]; u_ref = zeros(5,1);
            %z_ref_iter = [repmat(x_ref,6,1); u_ref(1:(5+1-1))];
            ...
            %z=fmincon(fun,zk,C_ineq1, C_ineq2, C_eq1, C_eq2, [], [], [], options);
